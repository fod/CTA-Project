...
The five algorithms described above are implemented in \texttt{main.py} as \texttt{insertion\_sort()}, \texttt{quicksort()}, \texttt{heapsort()}, \texttt{counting\_sort()}, and \texttt{introsort()}. All take as input a Python list of comparables, i.e. anything that implements the \texttt{\_\_lt\_\_}, \texttt{\_\_gt\_\_}, and \texttt{\_\_eq\_\_} methods. Some of the sorting functions require additional input. These functions have been so arranged, using wrapper functions or defaults, as to be callable using only an input list, to simplify automated benchmarking. Where an algorithm, for one reason or another, uses more than one function, the second function is defined as in inner function of the first. This is mainly to keep the code tidy and manageable. An exception to this is the \texttt{partition()} function which comprises the bulk of the quicksort algorithm, and which is defined as a separate top-level function, because it must also be available to \texttt{introsort()}.

Benchmarking is handled by the \texttt{benchmark()} function. This takes a list of function names, a list of integers representing the sizes of the lists of unsorted random integers to be used as test data, and an integer representing the number of times each sort should be repeated
