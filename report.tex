...
One final interesting optimisation, attributed without citation by \textcite[121]{bentley:pearls} to Sedgewick, is the use of inserion sort to sort small sub-arrays for which quicksort would be inefficient. The implementation suggested would entail halting partitioning when array length fell below a certain threshold, and sorting all of those short arrays together when all the larger ones had been mostly sorted by the partitioning algorithm.


\subsection{Heapsort}

Heapsort is a sorting algorithm that sorts data by first rearranging it so that it forms a \emph{max-heap}
