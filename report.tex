...
Quicksort is a comparison sorting algorithm which recursively partitions the input data based on whether each element is higher or lower than a (often arbitrarily chosen) \emph{pivot} value. The algorithm runs in $n (log(n))$ time in the best and average cases, and $n^{2}$

Figure \ref{fig:quicksort_1} illustrates the partitioning mechanism of the algorithm. First a pivot is chosen. Much can be said on the subject of pivot selection, and the matter is discussed below, but for the purposes of this demonstration, the pivot is taken to be the last element of the input array. The pivot value is highlighted in blue.
